...
Usage:
    ...
    ./ww #
    ```
    2b.) Run the program using the following command for file inputs ("name.txt" should be the filename to wrap): 
    ```
    ./ww # name.txt
    ```
    2c.) Run the program using the following command for directory inputs ("dir" should be the directory containing the files to wrap): 
    ```
    ./ww # dir
    ```
    
Note: The output files begin with "wrap." so the program does not recognize any files starting with "wrap."
