...
int main(int argc, char* argv[]){
    ...
    int byte;                                               // used for the while loop and used to detect end of file
    int fd; 
                                                        //file descriptor to keep track of the opened file, if any are open
    ...
    while(byte > 0) {
        ...
        if(!isspace(currentChar)) {                             // this case is for whenver we encounter a character that is a not a whitespace
            struct word newWord;  // initializes a word struct to tokenize and save the word we're about to encounter
            newWord.size = wrapLen;
            newWord.currentLength = 0;
            if((getNextWord(fd, &currentChar, &byte, &isBigWord, &newWord)) == 1){ 
                free(currentLine.characters);
                return EXIT_FAILURE;
            }

            if(isBigWord) isBigWord_Return = TRUE;
           ...
           if(isBigWord == TRUE) {                         // case to handle words that are bigger than wrapLen
            ...
            else if((byte == 0) && (currentLine.length > 0)) {                                  // this is for when we reach end of file. If there's anything in the current Line, write it out
                write(0,currentLine.characters, currentLine.length);
                currentLine.length = 0;
                printChar('\n');
            }
